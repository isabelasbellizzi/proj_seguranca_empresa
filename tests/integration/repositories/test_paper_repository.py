...
class TestPaperRepository (RepositoryBase):
    ...
    def test_Aget_byname_not_found(self) -> None:
        paper_name = ""
        resp = self.repo.get_byname(name=paper_name)
        assert resp is None

    def test_Bget_byname_found(self) -> None:
        inserted_paper = self.insert_manually.paper(session=self.session) # type: ignore
        paper_name = inserted_paper.name
    ...
    def test_Cget_not_found(self) -> None:
        paper_id = -1
        resp = self.repo.get(paper_id=paper_id)
        assert resp is None

    def test_Dget_found(self) -> None:
        inserted_paper = self.insert_manually.paper(session=self.session)# type: ignore
    ...
    def test_Eget_all_max_records(self):
    ...
    def test_Fget_all_system_id(self):
    ...
    def test_Jdelete_not_found_error(self) -> None:
    ...
    def test_Kdelete_found(self) -> None:
        inserted_paper = self.insert_manually.paper(session=self.session) # type: ignore
        # act
